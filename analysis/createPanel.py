...
def main():
    ...
    # Use recipient_name as both code and name since recipient_code is missing
    rec_code = pick(df, ["recipient_code", "recipient_name"])  # fallback to name if code missing
    rec_name = pick(df, ["recipient_name"])
    
    # If both are the same column, use only one for grouping
    ...
    # --- Build recipient×year panel (biodiversity slice only) ---
